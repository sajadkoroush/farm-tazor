...
contract BnbStaking is Ownable {
    ...
    // The block number when TAZOR mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 tazorReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accTazorPerShare = accTazorPerShare.add(tazorReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accTazorPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function deposit() public payable {
        ...
        updatePool(0);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accTazorPerShare).div(1e12).sub(user.rewardDebt);
            if(pending > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accTazorPerShare).div(1e12);
