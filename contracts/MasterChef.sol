...
// MasterChef is the master of Tazor. He can make Tazo and he is a fair guy.
//
...
contract MasterChef is Ownable {
    ...
    // Update the given pool's TAZOR allocation point. Can only be called by the owner.
    ...
    function pendingTazor(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.amount.div(20000).mul(accTazorPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        tazor.mint(address(taz), tazorReward);
        tazor.burn(address(taz), FeeTokenForBurn);
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'deposit TAZOR by staking');
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        ...
        require (_pid != 0, 'withdraw Tazor by unstaking');
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
    ...
    // Stake TAZOR tokens to MasterChef
    function enterStaking(uint256 _amount) public {
        ...
        taz.mint(msg.sender, _amount);
        taz.burn(msg.sender, _amount.mul(feeburnPercent).div(100));
        emit Deposit(msg.sender, 0, _amount);
    }

    // Withdraw TAZOR tokens from STAKING.
    ...
    function safeTazorTransfer(address _to, uint256 _amount) internal {
        taz.safeTazTransfer(_to, _amount);
    }
